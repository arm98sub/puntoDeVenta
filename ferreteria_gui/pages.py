...
from .config import PAGE_SIZE, TICKET_ROOT
...
class PosPage(QWidget):
    def __init__(self, database, parent=None):
        ...
        scan=QHBoxLayout(); scan.addWidget(QLabel("Buscar / escanear:")); self.barcode=QLineEdit(); self.barcode.setPlaceholderText("Barcode, código Truper, clave o descripción · Enter"); self.barcode.setStyleSheet("font-size:20px;padding:12px"); scan.addWidget(self.barcode,1)
...
class PaginatedProductsPage(QWidget):
    ...
    def __init__(self,database,title,headers,sort_columns,parent=None):
        ...
        bar=QHBoxLayout(); self.query=QLineEdit(); self.query.setPlaceholderText("Buscar o escanear producto..."); search=QPushButton("Buscar");self.filter=QComboBox();[self.filter.addItem(label,value) for label,value in self.FILTERS];bar.addWidget(self.query,1);bar.addWidget(search);bar.addWidget(QLabel("Filtro:"));bar.addWidget(self.filter);root.addLayout(bar)
