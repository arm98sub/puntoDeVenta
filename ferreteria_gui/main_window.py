...
class MainWindow(QMainWindow):
    def __init__(self,database,backup_root=None):
        ...
        self.nav.clear();self.nav.addItems(["Punto de venta","Productos y precios","Existencias y movimientos","Historial de ventas","Configuración"])
        self.stack=QStackedWidget(); self.pos=PosPage(database);self.products=ProductsPage(database);self.inventory=InventoryPage(database);self.history=HistoryPage(database);self.settings=SettingsPage(database,backup_root);[self.stack.addWidget(page) for page in (self.pos,self.products,self.inventory,self.history,self.settings)];root.addWidget(self.nav);root.addWidget(self.stack,1);outer.addLayout(root,1);self.setCentralWidget(central);self.settings.settings_saved.connect(self._apply_settings);self.settings.restored.connect(self._refresh_all);self._apply_settings(BusinessConfigService(database).obtener())
    ...
    def _navigate(self,index):
        ...
        if index==0:QTimer.singleShot(0,self.pos.focus_scanner)
        elif index in (1,2):self.stack.currentWidget().reload();self.stack.currentWidget().focus_search_deferred()
        elif index==3:self.stack.currentWidget().refresh()
        elif index==4:self.stack.currentWidget().load()
    ...
    def _refresh_all(self):
        self.products.page=1;self.products.reload();self.inventory.page=1;self.inventory.reload();self.history.refresh();self._apply_settings(BusinessConfigService(self.database).obtener());self.nav.setCurrentRow(0)
