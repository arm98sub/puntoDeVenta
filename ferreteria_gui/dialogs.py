...
from ferreteria_core.services import CategoryService,PurchasePresentationService,SupplierService
...
class ProductModifyDialog(QDialog):
    ...
    def _load_general_catalogs(self):
        ...
        self.purchase_presentation.addItem("Sin presentación habitual",None)
        for item in PurchasePresentationService(self.service.database).listar_activas():self.purchase_presentation.addItem(item.nombre,item.id)
    ...
    def _save(self):
        try:
            ...
            self.saved_product=self.service.modificar_producto(self.product.id,descripcion=self.description.text(),tipo_venta=self.kind.currentText(),unidad_granel=new_unit,precio_catalogo_publico=_optional_decimal(self.catalog.text()),precio_proveedor=_optional_decimal(self.cost.text()),porcentaje_ganancia=self.margin.text().strip() or None,precio_venta=_optional_decimal(self.sale.text()),controla_inventario=self.control.isChecked(),activo=self.active.isChecked(),precio_variable=self.variable.isChecked(),**extra)
            if not TRUPER_ENABLED:
                presentation_id=self.purchase_presentation.currentData();content=None
                if presentation_id:content=_whole_stock(self.purchase_content.text()) if self.kind.currentText()=="UNIDAD" else cantidad_desde_mayor(self.purchase_content.text(),new_unit)
                self.saved_product=self.service.configurar_presentacion_compra(self.product.id,presentation_id,content)
            self.accept()
...
class QuickProductDialog(QDialog):
    ...
    def _load_catalogs(self,category_id=None,supplier_id=None):
        ...
        self.purchase_presentation.clear();self.purchase_presentation.addItem("Sin presentación habitual",None)
        for item in PurchasePresentationService(self.database).listar_activas():self.purchase_presentation.addItem(item.nombre,item.id)
    ...
    def _save(self):
        try:
            ...
            if self.mode.currentData()=="EXTERNAL":
                ...
                if not TRUPER_ENABLED and self.purchase_presentation.currentData():
                    content=_whole_stock(self.purchase_content.text()) if kind=="UNIDAD" else cantidad_desde_mayor(self.purchase_content.text(),unit)
                    self.product=self.service.configurar_presentacion_compra(self.product.id,self.purchase_presentation.currentData(),content)
            else:
